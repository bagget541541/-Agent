...
def main() -> int:
    ...
    body = render_markdown(raw, image_root, args.embed_images)
    title = next((x[2:].strip() for x in raw.splitlines() if x.startswith("# ")), source.stem)
    fragment = f'<div style="max-width:680px;margin:0 auto;padding:20px 16px 32px;background:#fff;font-family:PingFang SC,Microsoft YaHei,sans-serif;line-height:1.8;color:#334155">{body}<div style="margin-top:28px;padding:10px 12px;background:#fff7ed;border:1px solid #fed7aa;border-radius:6px;text-align:center;color:#9a3412;font-size:11px;line-height:1.7">本内容由 AI 生成整理，请遵循相关法律法规及《人工智能生成合成内容标识办法》使用与传播。</div></div>'
    ...
    meta_path.write_text(json.dumps({"source": str(source.resolve()), "title": title, "output": str(out.resolve()), "embed_images": args.embed_images}, ensure_ascii=False, indent=2), encoding="utf-8")
