...
def _guess_category_from_wechat(title: str, text: str = "") -> str:
    """[已迁移至 common/classifier] 向后兼容包装。"""
    from common.classifier import classify_item
    return classify_item(title, text).get("category", "其他")


def _format_article_structured(article: dict, category: str) -> dict:
    """[已迁移至 common/normalizer] 向后兼容包装。"""
    from common.normalizer import _build_structured_for_category, _build_image_structured
    title = article.get("title", "")
    text = article.get("content_text") or article.get("full_text") or ""
    images = article.get("images", [])
    # 图片类文章
    if not text.strip() and images:
        return _build_image_structured(title, article.get("raw_text", ""), category)
    raw_text = article.get("raw_text", "")
    author = article.get("author") or article.get("account_name") or ""
    url = article.get("url", "")
    return _build_structured_for_category(category, title, text, raw_text, author, url)


def _build_image_article_structured(title: str, ocr_text: str, category: str) -> dict:
    """[已迁移至 common/normalizer] 向后兼容包装。"""
    from common.normalizer import _build_image_structured
    return _build_image_structured(title, ocr_text, category)
...
def step1_fetch_wechat(urls: list[str] = None) -> list[dict]:
    ...
    if not new_urls:
        if cached_articles:
            ...
            if not matched:
                print(f"  [Warning] None of {len(urls)} input URLs found in cache")
                return []
            # 检查并尝试重抓正文缺失的缓存条目
...
def _refine_article_title(raw_title: str, bank: str, category: str,
                          structured: dict, text: str = "") -> str:
    """[已迁移至 common/display_fields] 向后兼容包装。"""
    from common.display_fields import generate_display_fields
    result = generate_display_fields(
        bank=bank, category=category, structured=structured or {},
        raw_title=raw_title, raw_text=text,
    )
    return result["title"]


def _generate_title(bank: str, category: str, structured: dict,
                    text: str = "", fallback_title: str = "") -> str:
    """[已迁移至 common/display_fields] 向后兼容包装。"""
    from common.display_fields import generate_display_fields
    result = generate_display_fields(
        bank=bank, category=category, structured=structured or {},
        raw_title=fallback_title or "", raw_text=text,
    )
    return result["title"]
